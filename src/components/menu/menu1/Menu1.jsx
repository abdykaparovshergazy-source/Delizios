...
function Menu1({ cartItems, setCartItems }) {
  const [menu, setMenu] = useState([]);
  const [active, setActive] = useState("All");
  const [showAll, setShowAll] = useState(false);
  const [ratings, setRatings] = useState({});
  const [addedItems, setAddedItems] = useState({});
  const [selectedItem, setSelectedItem] = useState(null); // <-- modal үчүн
  ...
  const filteredMenu = active === "All"
    ? menu
    : menu.filter(item => item.category?.trim().toLowerCase() === active.toLowerCase());

  const visibleMenu = active === "All" && !showAll
  ...
  return (
    <>
      <section className="menu1 container" ref={topRef}>
        <h2 className="menu1-title">Menu</h2>

        <div className="menu1-tabs">
          {categories.map(cat => (
            <button
              key={cat}
        ...
        <div className="menu1-grid">
          {visibleMenu.map(item => (
            <div className="menu1-card" key={item.id}>
              <div className="menu1-img" onClick={() => setSelectedItem(item)}>
