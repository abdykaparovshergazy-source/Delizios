      ...
      <div className="menu1-grid">
        {visibleMenu.map(item => (
          <div className="menu1-card" key={item.id}>
            <div className="menu1-img"><img src={item.image} alt={item.title} /></div>
            <h3>{item.title}</h3>

            <div className="stars">
              {[1,2,3,4,5].map(star => (
                <span key={star} className={star <= (ratings[item.id]||0) ? "star active":"star"} onClick={() => handleRating(item.id, star)}>★</span>
              ))}
            </div>
